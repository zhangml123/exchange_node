...
class HuobiTicker {
	...
	getOrderBook(){
		...
		try{
						    ...
						    if(data1.tick){
						    	...
						    	if(a_arr[0] && b_arr[0]){
									var highestBid = b_arr[0];
						 			...
						 			self.params.tickers.huobiLowestAsk[pair] = JSON.stringify(lowestAsk);
						 			self.params.tickers.huobiHighestBid[pair]  = JSON.stringify(highestBid);
									var main = new Main(self.params); 
									main.refreshData();
								}
						    }else{
						   		self.params.tickers.huobiLowestAsk[pair]  = null;
						 		self.params.tickers.huobiHighestBid[pair]  = null;
						    }
						}
					})
				//}else{
			...
			self.params.tickers.huobiLowestAsk = null;
			self.params.tickers.huobiHighestBid = null;
