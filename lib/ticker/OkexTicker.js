	...
	getOrderBook(){
		...
		try{
				...
				if(frequency == 10 ){
					...
					if(a_arr[0] && b_arr[0]){
						var highestBid = b_arr[0];
			 			...
			 			self.params.orders.okexLowestAsk[pair]  = JSON.stringify(lowestAsk);
			 			self.params.orders.okexHighestBid[pair]  = JSON.stringify(highestBid);
			...
			ws.on('close', function close() {
				console.log(symbol)
				self.params.orders.okexLowestAsk[pair]  = null;
			 	self.params.orders.okexHighestBid[pair]  = null;
